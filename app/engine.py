...
from .model import MultiIsoForest
from .rules import (
    apply_simple_delta_rules,
    apply_temperature_baseline_rules,
)

# Optional rules (guarded imports so the engine still runs if they’re missing)
try:
...
class Engine:
    ...
    def process_tick(self, ts: datetime):
        ...
        # 5) Fit/score per metric, then apply rules
        scored_metrics = 0
        for metric, g in latest.groupby("metric"):
            ...
            # Train per-metric model on historical feats (neighbor_gap not required for training)
            ...
            # Neighbor coherence snapshot (kept for observability)
            ...
            # Rules: use raw values/timestamps for checks
            window_df = feat_m[["ts", "station_id", "metric", "value"]].copy()
            t_cur = pd.to_datetime(g.ts.iloc[0])

            if metric == "temperature":
                # Only the new ToD-baseline temperature rules (avoid duplicate alerts)
